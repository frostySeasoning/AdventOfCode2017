// n + s = 0
// nw + ne = n, sw + se = s, n + (sw || se) = (nw || ne) = 1
// analog für alle Himmelsrichtungen
// hr = [n, ne, se, s, sw, nw]
// if(abstand zw hr1 u hr2 == 3) -> 0
// if(abstand zw hr1 u hr2 == 2) -> 1

// cardinalDirections_ArrStr = ["n", "ne", "se", "s", "sw", "nw"]
function getReducedPathLength(path_Int)
{
  var crCounts_ArrInt = [0,0,0,0,0,0];
  for(let i = 0; i < 6; i++)
  { 
    crCounts_ArrInt[i] = path_Int.filter(item => { return item == i }).length;
  }
  // nw + ne = n, sw + se = s etc
  for(let i1 = 0; i1 < 6; i1++)
  {
    var i2 = (i1 + 2) % 6;
    var ir = (i1 + 1) % 6;
    if(crCounts_ArrInt[i1] < crCounts_ArrInt[i2])
    {
      crCounts_ArrInt[ir] += crCounts_ArrInt[i1];
      crCounts_ArrInt[i2] -= crCounts_ArrInt[i1];
      crCounts_ArrInt[i1] = 0;
    }
    else
    {
      crCounts_ArrInt[ir] += crCounts_ArrInt[i2];
      crCounts_ArrInt[i1] -= crCounts_ArrInt[i2];
      crCounts_ArrInt[i2] = 0;
    }
  }
  // n + s = 0, ne + sw = 0, se + nw = 0
  for(let i = 0; i < 3; i++)
  {
    if(crCounts_ArrInt[i] < crCounts_ArrInt[i+3])
    {
      crCounts_ArrInt[i+3] -= crCounts_ArrInt[i];
      crCounts_ArrInt[i] = 0;
    }
    else
    {
      crCounts_ArrInt[i] -= crCounts_ArrInt[i+3];
      crCounts_ArrInt[i+3] = 0;
    }
  }
  // nw + ne = n, sw + se = s etc again
  for(let i1 = 0; i1 < 6; i1++)
  {
    var i2 = (i1 + 2) % 6;
    var ir = (i1 + 1) % 6;
  ...
  return crCounts_ArrInt.reduce((sum, item) => {return sum + item});
}

function getPath_ArrInt(path_ArrStr)
{
  return path_ArrStr.map(item => {
    if(item == "n")
    {
      return 0;
    }
...
function test111()
{
  var tests = [["ne,ne,ne", 3], ["ne,ne,sw,sw", 0], ["ne,ne,s,s", 2], ["se,sw,se,sw,sw", 3]];
  var passed_bool = true;
  tests.forEach(item => {
    var result = getReducedPathLength(getPath_ArrInt(item[0].split(",")));
    if(result != item[1])
    {
      passed_bool = false;
    }
    console.log("" + (result == item[1]) + "\t"+result+"\t("+item[1]+")");
  });
  console.log(passed_bool);
}

// 664 for the given input
function solve111()
{
  return getReducedPathLength(getPath_ArrInt(input.split(",")));
}

// 1447 for the given input
function solve112()
{
  var path_ArrInt = getPath_ArrInt(input.split(","));
  var l = path_ArrInt.length;
  var max = 0;
  for(let i = 1; i <= l; i++)
  {
    var result = getReducedPathLength(path_ArrInt.slice(0, i));
    if(max < result)
    {
      max = result;
    }
