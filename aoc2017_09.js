// 7616 for given input
function solve091()
{
  return getTotalGroupScore(getCleanedString(input), 1);
}

// 3838 for the given input
function solve092()
{
...
function getCleanedString(string)
{
  ...
  while(string.indexOf("<") != -1)
  {
    ...
    var i2 = string.indexOf(">") + 1;
    if(string[i2] == ",")
    {
      i2++;
    }
    string = string.slice(0, i1) + string.slice(i2);
  }
  return string;
}

function countGarbage(string)
{
  var count = 0;
  while(string.indexOf("!") != -1)
  {
...
function getTotalGroupScore(string, score)
{
  ...
  while(string.indexOf("{") != -1)
  {
    var ci = getClosingIndex(string, "{", "}");
    if(ci == -1)
    {
      break;
    }
    subgroupstrings.push(string.slice(string.indexOf("{"), ci+1));
    string = string.slice(ci+1);
  }
  
  var subgroupscores = subgroupstrings.map(subgroupstring => {return getTotalGroupScore(subgroupstring, score+1)});
  
  return subgroupscores.reduce((sum, score) => {return sum + score}) + score;
}

function getClosingIndex(string, openingStr, closingStr)
{  
  var index = string.indexOf(openingStr) + 1;
  if(index == 0)
  {
    return -1;
  }
  ...
  while(count > 0)
  {
    var oi = string.indexOf(openingStr, index);
    var ci = string.indexOf(closingStr, index);
    if(ci == -1)
    {
      return -1;
    }
...
function test091()
{
  var tests = [["{}", 1], ["{{{}}}", 6], ["{{},{}}", 5], ["{{{},{},{{}}}}", 16], ["{<a>,<a>,<a>,<a>}", 1], ["{{<ab>},{<ab>},{<ab>},{<ab>}}", 9], ["{{<!!>},{<!!>},{<!!>},{<!!>}}", 9], ["{{<a!>},{<a!>},{<a!>},{<ab>}}", 3]];
  tests = tests.map(item => {
    var result = getTotalGroupScore(getCleanedString(item[0]), 1);
    console.log((result == item[1]) + ":\t" + result + "\t( "+item[0]+" = "+item[1]+" )");
    return result == item[1];
  });
  
  return tests.reduce((acc, item) => {return acc && item})
}

function test092()
{
  var tests = [["<>", 0], ["<random characters>", 17], ["<<<<>", 3], ["<{!>}>", 2], ["<!!>", 0], ["<!!!>>", 0], ["<{o\"i!a,<{i<a>", 10]];
  tests = tests.map(item => {
    var result = countGarbage(item[0]);
    console.log((result == item[1]) + ":\t" + result + "\t( "+item[0]+" = "+item[1]+" )");
    return result == item[1];
  });
