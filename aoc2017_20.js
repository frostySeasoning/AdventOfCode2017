...
function solve192()
{
  ...
  var particles_Arr = input.split("\n").map(item => {
    return item.split(";").map(item_ => {
      return item_.split(",").map(item__ => {
        return parseInt(item__, 10);
      });
    });
  });
  var i = 0;
  while(true)
  {
    particles_Arr = getUpdatedParticles(particles_Arr);
    if( i >= 1000)
    {
      // check if there are going to be more collisions
      // true -> i = 0
      // false -> return particles.length
      return particles_Arr.length;
    }
    i++;
  }
  return 0;
...
function getUpdatedParticles(particles_Arr)
{
  var updatedParticles_Arr = particles_Arr.slice();
  for(let i = 0, particlesCount = particles_Arr.length; i < particlesCount; i++)
  {
    // XYZ coordinates
    for(let i2 = 0; i2 < 3; i2++)
    {
      ...
      updatedParticles_Arr[i][1][i2] += updatedParticles_Arr[i][2][i2];
      // new_position = old_position + new_velocity
      updatedParticles_Arr[i][0][i2] += updatedParticles_Arr[i][1][i2];
    }
  }
  return getUnharmedParticles(updatedParticles_Arr);
}

// filters particles that collided
function getUnharmedParticles(particles_Arr)
{
  var collidedParticlesIndices_SetInt = new Set();
  for(let i = 0, particlesCount = particles_Arr.length; i < particlesCount - 1; i++)
  {
    /*if(collidedParticlesIndices_SetInt.has(i))
    {
      continue;
    }*/
    for(let i2 = i+1; i2 < particlesCount; i2++)
    {
      /*if(collidedParticlesIndices_SetInt.has(i2))
      {
        continue;
      }*/
      var xyz1 = particles_Arr[i][0];
      var xyz2 = particles_Arr[i2][0];
      if(xyz1[0] == xyz2[0] && xyz1[1] == xyz2[1] && xyz1[2] == xyz2[2])
      {
        collidedParticlesIndices_SetInt.add(i).add(i2);
      }
    }
  }
  return particles_Arr.filter((element, index) => { return !collidedParticlesIndices_SetInt.has(index) });
}

function areCollisionsPossible(particles_Arr, t_Int)
{
  for(let i = 0, particleCount = particles_Arr.length; i < particleCount - 1; i++)
  {
    for(let i2 = i + 1; i2 < particleCount; i2++)
    {
      // c1 + v1*t + a1*t*t == c2 + v2*t + a2*t*t
      // c = x || y || z
      // t1,2 = (-dv+-sqrt(dv*dv-4*da*dc))/(2*da)
      let radikand;
      if(radikand < 0)
      {
        continue;
...
var input = `p=<-1724,-1700,5620>, v=<44,-10,-107>, a=<2,6,-9>
